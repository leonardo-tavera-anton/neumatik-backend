...
        // 2. Insertar nuevo usuario. Corregido: Usa ultima_conexion
        const newUserQuery = `INSERT INTO usuarios (nombre, apellido, correo, contrasena_hash, telefono, es_vendedor, creado_en, ultima_conexion) VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW()) RETURNING id, nombre, apellido, correo, telefono, es_vendedor;`;
...
        // 4. Sanear y devolver respuesta (201 Created)
        // Sanitiza NULL a string vacío ("") para Flutter
        const perfil = {
            id: newUser.id,
            ...
            es_vendedor: newUser.es_vendedor,
        };
...
    try {
        // 1. Buscar usuario por correo y obtener el hash de la contraseña, además de los datos de perfil
        const userResult = await pool.query(
            'SELECT id, nombre, apellido, correo, telefono, es_vendedor, contrasena_hash FROM usuarios WHERE correo = $1', 
            [correo]
        );
        
...
        // 4. Actualizar ultima_conexion (¡Corregido!).
        await pool.query('UPDATE usuarios SET ultima_conexion = NOW() WHERE id = $1', [user.id]);
        ...
        const perfil = {
            id: user.id,
            nombre: user.nombre,
            apellido: user.apellido || "", // Si es NULL, se convierte a ""
            correo: user.correo,
            telefono: user.telefono || "", // Si es NULL, se convierte a ""
...
        }
        ...
        const perfil = {
            id: user.id,
            nombre: user.nombre,
            apellido: user.apellido || "", // Sanitiza NULL a ""
            correo: user.correo,
            telefono: user.telefono || "", // Sanitiza NULL a ""
            es_vendedor: user.es_vendedor,
        };
        
        res.json({ 
